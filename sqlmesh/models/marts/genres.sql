...
WITH exploded_genres AS (
    ...
    SELECT
        CAST(g.value->>'id' AS INTEGER) AS genre_id,
        g.value->>'name' AS genre_name,
        g.value->>'slug' AS genre_slug,
        ga.id AS game_id,
    ...
    CROSS JOIN json_each(ga.genres) AS g
...
LEFT JOIN staging.stg_genres g ON eg.genre_id = g.id
GROUP BY
        eg.genre_id,
        eg.genre_name,
        eg.genre_slug,
