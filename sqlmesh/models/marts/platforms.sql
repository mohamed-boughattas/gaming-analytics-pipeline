...
WITH exploded_platforms AS (
    ...
    SELECT
        CAST(p.value->'platform'->>'id' AS INTEGER) AS platform_id,
        p.value->'platform'->>'name' AS platform_name,
        p.value->'platform'->>'slug' AS platform_slug,
        ga.id AS game_id,
        ...
        ga.released,
        ga.playtime
    FROM staging.stg_games ga
    CROSS JOIN json_each(ga.platforms) AS p
...
LEFT JOIN staging.stg_platforms p ON ep.platform_id = p.id
GROUP BY
        ep.platform_id,
        ep.platform_name,
        ep.platform_slug,
